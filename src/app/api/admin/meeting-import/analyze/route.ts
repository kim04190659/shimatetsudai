...
import { resolveIssueRelationTargets, getPageTextContent, structureMeetingNotes } from "@/lib/adminMeetingImport";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const issuePageId = body.issuePageId?.trim();
    const meetingNotePageId = body.meetingNotePageId?.trim();

    if (!issuePageId || !meetingNotePageId) {
